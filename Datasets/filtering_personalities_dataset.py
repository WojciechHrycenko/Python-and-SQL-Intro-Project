...
df_sampled = df_filtered.groupby('country').apply(lambda x: x.sample(1000))

# Zapisz przefiltrowane dane do nowego pliku CSV
df_sampled.to_csv('cleaned_big_personalities_dataset.csv', index=False)

print("Nowy plik został utworzony z maksymalnie 1000 losowymi wierszami dla każdego wybranego kraju.")
